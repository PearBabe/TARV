...
static aflnet_timing_trace_t timing_trace = {NULL, 0};

static unsigned long long aflnet_now_ms(void) {
  struct timeval tv;
  gettimeofday(&tv, NULL);
  return (unsigned long long)tv.tv_sec * 1000ULL + (unsigned long long)(tv.tv_usec / 1000ULL);
}

static void aflnet_append_timing_request(unsigned int len) {
  unsigned int next = timing_trace.count + 1;
  timing_trace.events = (aflnet_timed_event_t *)ck_realloc(
      timing_trace.events, next * sizeof(aflnet_timed_event_t));

  aflnet_timed_event_t *event = &timing_trace.events[timing_trace.count];
  memset(event, 0, sizeof(*event));
  event->request_sent_ms = aflnet_now_ms();
  event->request_len = len;
  event->gap_prev_ms = -1;
  if (timing_trace.count > 0) {
    aflnet_timed_event_t *prev = &timing_trace.events[timing_trace.count - 1];
    event->gap_prev_ms = (long long)event->request_sent_ms - (long long)prev->request_sent_ms;
  }
  timing_trace.count = next;
}

static void aflnet_append_timing_response_chunk(unsigned int len) {
  if (!timing_trace.count) return;

  aflnet_timed_event_t *event = &timing_trace.events[timing_trace.count - 1];
  unsigned long long now = aflnet_now_ms();
  if (!event->response_seen) {
    event->response_first_ms = now;
    event->response_seen = 1;
  }
  event->response_done_ms = now;
  event->response_len += len;
}

static void aflnet_set_timing_response_outcome(unsigned char outcome) {
  if (!timing_trace.count || outcome == AFLNET_TIMING_OUTCOME_NONE) return;
  aflnet_timed_event_t *event = &timing_trace.events[timing_trace.count - 1];
  event->response_outcome = outcome;
  event->response_outcome_ms = aflnet_now_ms();
}

static unsigned char aflnet_classify_socket_errno(int errnum) {
  switch (errnum) {
    case ETIMEDOUT:
      return AFLNET_TIMING_OUTCOME_TIMEOUT;
    case ECONNRESET:
    case ECONNABORTED:
    case ENETRESET:
    case EPIPE:
    case ENOTCONN:
      return AFLNET_TIMING_OUTCOME_RESET;
    default:
      return AFLNET_TIMING_OUTCOME_IO_ERROR;
  }
}

static unsigned char aflnet_socket_outcome_or_default(int sockfd,
                                                       unsigned char default_outcome) {
  int so_error = 0;
  socklen_t so_len = sizeof(so_error);
  if (getsockopt(sockfd, SOL_SOCKET, SO_ERROR, &so_error, &so_len) == 0 &&
      so_error != 0) {
    return aflnet_classify_socket_errno(so_error);
  }
  return default_outcome;
}

static unsigned char aflnet_connection_close_outcome(int sockfd) {
  if (timing_trace.count) {
    const aflnet_timed_event_t *event = &timing_trace.events[timing_trace.count - 1];
    /* If the peer aborts the connection before any response bytes arrive,
       treat it as reset-like rather than an orderly close. */
    if (!event->response_seen && event->response_len == 0) {
      return aflnet_socket_outcome_or_default(sockfd, AFLNET_TIMING_OUTCOME_RESET);
    }
  }
  return aflnet_socket_outcome_or_default(sockfd, AFLNET_TIMING_OUTCOME_CLOSE);
}

void aflnet_clear_timing_trace(void) {
  if (timing_trace.events) ck_free(timing_trace.events);
  timing_trace.events = NULL;
  timing_trace.count = 0;
}

const aflnet_timing_trace_t* aflnet_get_timing_trace(void) {
  return &timing_trace;
}
...
region_t* extract_requests_sip(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if ((mem_count > 1) && ((mem[mem_count - 1] == terminator[0]) || (mem[mem_count - 1] == terminator[1])) &&
  ...
  if (rv > 0) {
    if (pfd[0].revents & POLLOUT) {
      while (byte_count < len) {
        ...
        if (n == -1) {
          aflnet_set_timing_response_outcome(aflnet_classify_socket_errno(errno));
          return -1;
        }
        byte_count += n;
      }
      if (byte_count > 0) aflnet_append_timing_request(byte_count);
    }
  }
  return byte_count;
}
...
int net_recv(int sockfd, struct timeval timeout, int poll_w, char **response_buf, unsigned int *len) {
  ...
  if (rv > 0) {
    if (pfd[0].revents & POLLNVAL) {
      aflnet_set_timing_response_outcome(AFLNET_TIMING_OUTCOME_IO_ERROR);
      return 1;
    }
    if (pfd[0].revents & POLLERR) {
      aflnet_set_timing_response_outcome(
          aflnet_socket_outcome_or_default(sockfd, AFLNET_TIMING_OUTCOME_IO_ERROR));
      return 1;
    }
    if (pfd[0].revents & POLLIN) {
      n = recv(sockfd, temp_buf, sizeof(temp_buf), 0);
      if (n == 0) {
        aflnet_set_timing_response_outcome(aflnet_connection_close_outcome(sockfd));
        return 0;
      }
      if ((n < 0) && (errno != EAGAIN)) {
        aflnet_set_timing_response_outcome(aflnet_classify_socket_errno(errno));
      ...
      while (n > 0) {
        usleep(10);
        aflnet_append_timing_response_chunk((unsigned int)n);
  ...
  if (rv == 0) {
    aflnet_set_timing_response_outcome(AFLNET_TIMING_OUTCOME_TIMEOUT);
  }
