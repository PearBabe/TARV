...
typedef struct {
  unsigned long long request_sent_ms;    /* request send completion time */
  unsigned long long response_first_ms;  /* first response byte time, 0 if none */
  unsigned long long response_done_ms;   /* final response byte time, 0 if none */
  unsigned long long response_outcome_ms;/* close/reset/timeout detection time, 0 if none */
  long long gap_prev_ms;                 /* gap to previous request send, -1 if none */
  unsigned int request_len;              /* bytes sent in this request */
  unsigned int response_len;             /* bytes received after this request */
  unsigned char response_seen;           /* whether any response bytes were observed */
  unsigned char response_outcome;        /* terminal receive outcome classification */
} aflnet_timed_event_t;

typedef struct {
  aflnet_timed_event_t *events;          /* timed request/response observations */
  unsigned int count;                    /* number of valid events */
} aflnet_timing_trace_t;

enum {
  AFLNET_TIMING_OUTCOME_NONE = 0,
  AFLNET_TIMING_OUTCOME_TIMEOUT = 1,
  AFLNET_TIMING_OUTCOME_CLOSE = 2,
  AFLNET_TIMING_OUTCOME_RESET = 3,
  AFLNET_TIMING_OUTCOME_IO_ERROR = 4,
};
...
/* Timed request/response trace helpers for Bi-ZoneFuzz++ style instrumentation. */
