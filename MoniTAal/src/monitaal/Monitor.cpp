...
namespace monitaal {
    ...
    Single_monitor<state_t>::status() const { return _status; }
    ...
    template<class state_t> std::vector<state_t>
    Single_monitor<state_t>::state_estimate() const { return _current_states; };

    std::ostream& operator<<(std::ostream& out, const single_monitor_answer_e value) {
        switch (value) {
    ...
    template<class state_t>
    std::string Single_monitor<state_t>::feedback_summary() const {
    ...
    template<class state_t>
    std::vector<state_t>
    Monitor<state_t>::positive_state_estimate() const {
        return _monitor_pos.state_estimate();
    }

    template<class state_t>
    std::vector<state_t>
    Monitor<state_t>::negative_state_estimate() const {
        return _monitor_neg.state_estimate();
    }

    template<class state_t>
    single_monitor_answer_e Monitor<state_t>::positive_monitor_status() const {
        return _monitor_pos.status();
    }

    template<class state_t>
    single_monitor_answer_e Monitor<state_t>::negative_monitor_status() const {
        return _monitor_neg.status();
    }

    template<class state_t>
    std::string Monitor<state_t>::feedback_summary() const {
        std::ostringstream out;
        out << "verdict=" << _status
