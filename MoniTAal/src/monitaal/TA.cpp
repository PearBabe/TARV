...
namespace monitaal {
    ...
    std::string TA::name() const { return _name; }

    std::string TA::clock_name(clock_index_t index) const { return _clock_names.at(index); }

    const clock_map_t& TA::clocks() const { return _clock_names; }
    ...
    size_t TA::location_count() const { return _locations.size(); }

    size_t TA::edge_count() const {
        size_t count = 0;
        for (const auto& [_, edges] : _forward_edges)
            count += edges.size();
        return count;
    }

    const location_t& TA::location(location_id_t id) const { return _locations.at(id); }

    std::vector<location_id_t> TA::accepting_locations() const {
        std::vector<location_id_t> accepting;
        for (const auto& [id, loc] : _locations)
            if (loc.is_accept())
                accepting.push_back(id);
        return accepting;
    }
    ...
    std::vector<label_t> TA::sorted_labels() const {
        std::vector<label_t> labels(_labels.begin(), _labels.end());
    ...
    namespace {
        ...
        std::string constraints_summary(const constraints_t& constraints) {
            std::ostringstream out;
            out << "[";
            bool first = true;
            for (const auto& c : constraints) {
                ...
                out << c._i << "-" << c._j << bound_summary(c._bound);
            }
            out << "]";
            return out.str();
        }

        std::string clocks_summary(const clocks_t& clocks) {
            std::ostringstream out;
            out << "[";
            bool first = true;
    ...
    std::string TA::stable_summary() const {
        std::ostringstream out;
        out << "TA{name=" << _name
            << ";initial=" << _initial
            << ";clocks=[";
        bool first = true;
        for (const auto& [index, name] : _clock_names) {
            if (first) first = false;
            else out << ",";
            out << index << ":" << name;
        }
        ...
        for (const auto& [id, loc] : _locations) {
            ...
            out << id << ":" << loc.name() << ":accept=" << loc.is_accept()
                << ":inv=" << constraints_summary(loc.invariant());
        }
        out << "];edges=[";
        first = true;
        for (const auto& [from, edges] : _forward_edges) {
            for (const auto& edge : edges) {
                if (first) first = false;
                else out << ",";
                out << from << "->" << edge.to()
                    << ":label=" << edge.label()
                    << ":guard=" << constraints_summary(edge.guard())
                    << ":reset=" << clocks_summary(edge.reset());
            }
        }
        out << "]}";
        return out.str();
