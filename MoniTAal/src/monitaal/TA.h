...
namespace monitaal {
    ...
    public:
        ...
        [[nodiscard]] const clock_map_t& clocks() const;

        std::map<location_id_t, std::vector<clock_index_t>> inactive_clocks() const;

        [[nodiscard]] const location_map_t& locations() const;

        [[nodiscard]] size_t location_count() const;

        [[nodiscard]] size_t edge_count() const;

        [[nodiscard]] const location_t& location(location_id_t id) const;

        [[nodiscard]] std::vector<location_id_t> accepting_locations() const;
        ...
        [[nodiscard]] std::string stable_summary() const;
