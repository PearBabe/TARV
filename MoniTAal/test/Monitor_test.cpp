...
BOOST_AUTO_TEST_CASE(monitor_feedback_helpers_are_stable_and_read_only) {
    TA pos = Parser::parse_file("models/a-b.xml", "a_leadsto_b");
    TA neg = Parser::parse_file("models/a-b.xml", "not_a_leadsto_b");

    BOOST_CHECK_EQUAL(pos.name(), "a_leadsto_b");
    BOOST_CHECK(pos.location_count() > 0);
    BOOST_CHECK(pos.edge_count() > 0);
    BOOST_CHECK(pos.location(pos.initial_location()).id() == pos.initial_location());
    BOOST_CHECK(not pos.accepting_locations().empty());

    auto labels = pos.sorted_labels();
    BOOST_CHECK(std::is_sorted(labels.begin(), labels.end()));
    BOOST_CHECK(pos.stable_summary().find("TA{name=a_leadsto_b") == 0);
    BOOST_CHECK(pos.stable_summary().find(";edges=[") != std::string::npos);

    const Concrete_monitor monitor(pos, neg);
    BOOST_CHECK_EQUAL(monitor.status(), INCONCLUSIVE);
    BOOST_CHECK_EQUAL(monitor.positive_monitor_status(), ACTIVE);
    BOOST_CHECK_EQUAL(monitor.negative_monitor_status(), ACTIVE);
    BOOST_CHECK(not monitor.positive_state_estimate().empty());
    BOOST_CHECK(not monitor.negative_state_estimate().empty());

    auto feedback = monitor.feedback_summary();
    BOOST_CHECK(feedback.find("verdict=INCONCLUSIVE") != std::string::npos);
    BOOST_CHECK(feedback.find("positive={status=ACTIVE;states=[loc=") != std::string::npos);
    BOOST_CHECK(feedback.find("negative={status=ACTIVE;states=[loc=") != std::string::npos);
    BOOST_CHECK(feedback.find(";valuation=[") != std::string::npos);
}
