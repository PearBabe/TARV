    ...
    */
    ...
    std::pair<monitaal::TA, std::string> build_ta_from_main(MitlParser::MainContext* phi_) {
        ...
        */
        ...
        if (monitor_concrete_labels) {
            std::vector<std::pair<std::string, int>> ordered(nnf_formula->props.begin(), nnf_formula->props.end());
            ...
            for (const auto& [name, _] : ordered) {
                monitor_alphabet.push_back(name);
            }
        }
...
//        bdd_edges_.clear();
        ...
        if (!out_format.has_value() || (out_format.has_value() && out_flatten)) {
            ...
            back = false;
            last_intersection = !monitor_concrete_labels;
            ...
            if (monitor_concrete_labels) {
                return { product.projection_concrete(nnf_formula->props), std::string{} };
            }
