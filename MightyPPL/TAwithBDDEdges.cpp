...
namespace monitaal {
    ...
    namespace {
        ...
        using ordered_prop_t = std::pair<std::string, int>;

        std::vector<ordered_prop_t> ordered_props(const std::map<std::string, int>& props) {
            std::vector<ordered_prop_t> output(props.begin(), props.end());
        ...
        void expand_assignment(const std::string& pattern,
                               const std::vector<ordered_prop_t>& props,
                               size_t index,
                               std::string& current,
                               std::set<std::string>& labels) {
            if (index == props.size()) {
                labels.insert(current.empty() ? "-" : current);
                return;
            }

            const int bdd_index = props[index].second;
            const char value = bdd_index >= 0 && static_cast<size_t>(bdd_index) < pattern.size()
                    ? pattern[static_cast<size_t>(bdd_index)]
                    : 'X';

            if (value == '0' || value == '1') {
                current.push_back(value);
                expand_assignment(pattern, props, index + 1, current, labels);
                current.pop_back();
                return;
            }
 ...
 //                       std::cout << "bdd_satcount(e2.bdd_label()) == " << bdd_satcount(e2.bdd_label()) << std::endl;
                        if ((e1.bdd_label() & e2.bdd_label()) != bddfalse) {
...
//                            std::cout << "(" << l1 << ", " << l2 << ") syncs" << std::endl;
    ...
    TAwithBDDEdges TAwithBDDEdges::intersection(const std::vector<TAwithBDDEdges>& components) {
        ...
        for (size_t i = 0; i < components.size(); ++i) {
            ...
            for (size_t j = 0; j < components[i].number_of_clocks(); ++j) {
                if (j == 0) {
            ...
            if (mightypplcpp::out_fin) {
                ...
                reachable_state_space =
                        monitaal::Fixpoint<monitaal::symbolic_state_t>::reach(
                                monitaal::Fixpoint<monitaal::symbolic_state_t>::accept_states(projected),
                                projected);
                ...
                reachable_state_space =
                        monitaal::Fixpoint<monitaal::symbolic_state_t>::buchi_accept_fixpoint(projected);
                if (initial_state.is_included_in(reachable_state_space)) {
    ...
    TA TAwithBDDEdges::projection_concrete(const std::map<std::string, int>& props) {

        edges_t edges;
        bdd projected_e;
        bdd props_to_remove = bdd_true();
        std::set<int> prop_ids;
        for (const auto& [_, id] : props) {
            prop_ids.insert(id);
        }
        for (int i = 0; i < bdd_varnum(); ++i) {
            if (!prop_ids.count(i)) {
                props_to_remove = props_to_remove & bdd_ithvar(i);
            }
        }

        const auto props_ordered = ordered_props(props);

        for (const auto& [id, l] : this->locations()) {
            for (const auto& e : this->bdd_edges_from(id)) {
                projected_e = bdd_exist(e.bdd_label(), props_to_remove);
                bdd_allsat(projected_e, *mightypplcpp::allsat_print_handler);

                std::set<std::string> labels;
                for (const auto& pattern : mightypplcpp::sat_paths) {
                    std::string label;
                    expand_assignment(pattern, props_ordered, 0, label, labels);
                }
